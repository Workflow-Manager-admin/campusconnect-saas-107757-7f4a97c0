...
if [ -f "schema.sql" ]; then
    ...
    SCHEMA_EXISTS=$(sudo -u postgres ${PG_BIN}/psql -p ${DB_PORT} -d ${DB_NAME} -t -c "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'users';" 2>/dev/null || echo "0")
    ...
    echo "Warning: schema.sql not found - database schema not initialized"
fi
